...
export default function SettingsPage() {
  ...
  const [profile, setProfile] = useState({ full_name: '', email: '', username: '', phone: '', company_id: '', avatar: null, avatar_path: '' });
  ...
  function renderGeneralTab() {
    return (
      <div className="dashboard-grid">
        ...
        <SectionCard title="Master Data" subtitle="Companies, departments, and designations are editable here.">
        ...
        >
          <form id="profile-form" className="form-grid" onSubmit={saveProfile}>
            ...
            <label><span>Company</span>
              <select value={profile.company_id} onChange={(event) => setProfile((current) => ({ ...current, company_id: event.target.value }))}>
                <option value="">Select company</option>
                {companies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
            </label>
    ...
    <AppShell title="Settings" subtitle="Manage masters, notifications, users, and your account profile." breadcrumbs={['Home', 'Administration', 'Settings']}>
      ...
      >
        <form id="user-form" className="form-grid" onSubmit={saveUser}>
          ...
          <label><span>Company</span>
            <select value={userForm.company_id} onChange={(event) => setUserForm((current) => ({ ...current, company_id: event.target.value }))}>
              <option value="">Select company</option>
              {companies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
          </label>
