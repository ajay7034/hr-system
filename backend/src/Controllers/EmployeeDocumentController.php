...
final class EmployeeDocumentController
{
    ...
    public function delete(Request $request, array $params): void
    {
        ...
        if ($statement->rowCount() === 0) {
            Response::json(['success' => false, 'message' => 'Employee document not found.'], 404);
            return;
        }

        $cleanup = $this->pdo->prepare("DELETE FROM notifications WHERE related_table = 'employee_documents' AND related_id = :id");
        $cleanup->execute(['id' => $id]);

        $emailCleanup = $this->pdo->prepare("DELETE FROM email_logs WHERE related_table = 'employee_documents' AND related_id = :id");
        $emailCleanup->execute(['id' => $id]);

        $this->activityLogger->log(Auth::id(), 'employee_document', $id, 'deleted', 'Employee document deleted.');

        Response::json(['success' => true, 'message' => 'Employee document deleted.']);
    }
}
