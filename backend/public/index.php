...
use App\Controllers\EmployeeRequestController;
...
$globalSearchController = new GlobalSearchController($pdo);
$notificationController = new NotificationController($pdo);
$reportController = new ReportController($pdo);
$adminToolsController = new AdminToolsController($pdo, $config);
...
$router->add('GET', '/api/request-portal/employees', fn ($request) => $employeeRequestController->employeeSearch($request));
$router->add('POST', '/api/request-portal/requests', fn ($request) => $employeeRequestController->submit($request));
...
$router->add('POST', '/api/passports/{id}/delete', fn ($request, $params) => $passportController->delete($request, $params), [$auth, $adminOrHr]);
...
$router->add('POST', '/api/employee-documents/{id}/delete', fn ($request, $params) => $employeeDocumentController->delete($request, $params), [$auth, $adminOrHr]);

$router->add('GET', '/api/company-documents', fn ($request) => $companyDocumentController->index($request), [$auth]);
...
$router->add('GET', '/api/admin/activity-logs', fn ($request) => $adminToolsController->activityLogs($request), [$auth, $adminOnly]);
$router->add('GET', '/api/admin/backups/database', fn () => $adminToolsController->downloadDatabaseBackup(), [$auth, $adminOnly]);
...
$router->add('GET', '/api/requests', fn ($request) => $employeeRequestController->index($request), [$auth, $adminOrHr]);
$router->add('POST', '/api/requests/{id}/approve', fn ($request, $params) => $employeeRequestController->approve($request, $params), [$auth, $adminOrHr]);
