...
final class ReminderService
{
    ...
    public function generate(): array
    {
        ...
        $employeeDocuments = $this->pdo->query("
            SELECT ed.id, ed.expiry_date, ed.status, ed.alert_days, e.full_name, e.email, edm.name AS document_type
        ...
        $companyDocuments = $this->pdo->query("
            SELECT cd.id, cd.expiry_date, cd.status, cd.alert_days, cd.document_name, cdm.name AS document_type
        ...
        foreach ($employeeDocuments as $document) {
            ...
            if ($this->storeNotification('employee_documents', (int) $document['id'], $title, $message, $status === 'expired' ? 'critical' : 'warning')) {
                $createdNotifications++;
            }

            if (!empty($document['email']) && $this->queueEmail('employee_documents', (int) $document['id'], $document['email'], $title, $message)) {
                $queuedEmails++;
            }
        }
        ...
        foreach ($companyDocuments as $document) {
            ...
            if ($this->storeNotification('company_documents', (int) $document['id'], $title, $message, $status === 'expired' ? 'critical' : 'warning')) {
                $createdNotifications++;
            }
        }
