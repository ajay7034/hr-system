...
export default function ReportsPage() {
  ...
  const selectedCompanyId = Number(filters.company_id || 0);
  ...
  const availableBranches = useMemo(() => {
    ...
    return lookups.branches.filter((branch) => Number(branch.company_id) === selectedCompanyId);
  }, [lookups.branches, selectedCompanyId]);
  ...
  const loadReports = async (overrideFilters = filters) => {
    ...
    const common = {
      ...
      branch_id: overrideFilters.branch_id || '',
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <select name="branch_id" value={filters.branch_id} onChange={handleFilterChange}>
                ...
                {availableBranches.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
            </label>
